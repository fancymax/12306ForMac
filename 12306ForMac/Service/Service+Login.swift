...
extension Service {
    ...
    // MARK: - Request Flow
    func preLoginFlow(_ success:@escaping (_ loadImage:NSImage)->(),failure:@escaping (_ error:NSError)->()){
    ...
    func loginFlow(_ user:String, passWord:String, randCodeStr:String,success:@escaping ()->(), failure:@escaping (_ error:NSError)->()){
    ...
    func loginOut()
    {
        ...
        Service.Manager.request(url).responseString{ response in
        }
    }
    ...
    func initMy12306()->Promise<Void>{
        return Promise{ fulfill, reject in
            ...
            let headers = ["refer": "https://kyfw.12306.cn/otn/login/init"]
            Service.Manager.request(url, headers:headers).responseString {response in
                switch (response.result){
                case .failure(let error):
                    reject(error)
                case .success(let data):
